...
训练过程中loss的变化如~\ref{fig:goodmodel_wgan}所示。我们发现WGAN的判别器loss呈现抛物线的形状，在前中期不断上升，在后期开始一路下滑。在众多WGAN的文献中WGAN普遍呈现初期是较大的负数，之后稳定上升的情况，没有关于先升后降的请情况。判别器损失越趋向于负数，则其判别出生成图像的能力越强，虽然无法推断后期的下降是由什么引起，但是从生成结果~\ref{fig:wgan_evolve}中并没有看出明显的质量下降，网络没有出现训练崩溃的情况。
...
最终我们确定了WGAN使用$\lambda_{adv}=1$作为
